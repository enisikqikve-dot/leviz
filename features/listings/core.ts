...
import { canManage, type Actor } from '@/lib/auth/permissions';
...
/**
 * Inserate anlegen, veroeffentlichen, pausieren, verkaufen, loeschen -- ein
 * Weg fuer Website und App.
 *
 * Wer handelt, kommt als `Actor` herein: die Server Actions holen ihn aus dem
 * Cookie, die API aus dem Token. Die Pruefungen dahinter sind dieselben, und
 * das ist der Punkt: ein Inserat, das die App anlegt, geht durch dieselbe
 * Moderation, dieselben Paketgrenzen und denselben Qualitaetswert wie eines
 * von der Website.
 */
 ...
 * im Browser oder in der App ist Bequemlichkeit, kein Schutz.
 */
export async function saveListing(
  ...
  const limits = await getLimits(actor.id);
  ...
  if (vehicleId) {
    ...
    if (!canManage(actor, existing)) return fail('Nuk keni qasje në këtë shpallje');
  }
...
export async function publishListing(
  ...
  const loaded = await loadOwned(actor, vehicleId);
...
export async function toggleListingPause(
  actor: Actor,
  vehicleId: string,
...
export async function markListingSold(actor: Actor, vehicleId: string): Promise<ActionResult> {
  const loaded = await loadOwned(actor, vehicleId);
  if (!loaded.ok) return fail(loaded.error);
 ...
 */
export async function deleteListing(actor: Actor, vehicleId: string): Promise<ActionResult> {
  const loaded = await loadOwned(actor, vehicleId);
  if (!loaded.ok) return fail(loaded.error);
